...
export default function RecordsReporting() {
      ...
      try {
        ...
        const mapped = await Promise.all(
          (liveData || []).map(async (c) => ({
            ...
            days_active: Math.max(0, Math.floor((Date.now() - new Date(c.created_at).getTime()) / (1000 * 60 * 60 * 24))),
          }))
        );
        if (!isMounted) return;
        setRecords(mapped);
  ...
  const filteredRecords = useMemo(() => {
    return records.filter((r) => {
      if (selectedCrimeFilter !== 'ALL' && r.crime_type !== selectedCrimeFilter) return false;
      if (selectedStatusFilter !== 'ALL' && r.investigation_status !== selectedStatusFilter) return false;
      if (searchQuery.trim()) {
  ...
  const meanDuration = filteredRecords.length
    ? (filteredRecords.reduce((acc, r) => acc + r.days_active, 0) / filteredRecords.length).toFixed(1)
    : '0.0';

  const handleExportCSV = () => {
    const headers = ['Case_Token_SHA256', 'Case_Number', 'Crime_Head', 'Investigation_Stage', 'Days_Active', 'Court_Level', 'Bail_Status'];
        ...
        <div className="page-header">
          <div>
            ...
            <p className="page-desc">
              De-identified case metadata for statistical and longitudinal analysis. Complainant
              names, victim identities, witness contacts, and raw documents are structurally
              excluded from this endpoint — not filtered client-side, never returned by the server
              at all.
            </p>
          </div>
            ...
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap', flex: 1 }}>
              ...
              <div>
                ...
                >
                  ...
                  {INVESTIGATION_STAGES.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
                </select>
              </div>
            </div>
        ...
        <div className="card" style={{ padding: 0 }}>
          <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--color-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span className="table-caption" style={{ margin: 0, fontWeight: 500 }}>
              {loading ? 'Loading case metadata…' : `Showing ${filteredRecords.length} de-identified case${filteredRecords.length === 1 ? '' : 's'}`}
            </span>
            {!loading && !error && records.length > 0 && (
              <span style={{ fontSize: '12px', color: 'var(--color-text-tertiary)' }}>Click any row to inspect details</span>
            )}
          </div>
          ...
          <div className="table-container" style={{ border: 'none', borderRadius: 0 }}>
            <table className="data-table">
              ...
              <tbody>
                {error ? (
                  <tr>
                    <td colSpan="7" style={{ textAlign: 'center', padding: '32px', color: 'var(--color-status-error, #b91c1c)' }}>
                      {error}
                    </td>
                  </tr>
                ) : loading ? (
                  ...
                  <tr>
                    <td colSpan="7" style={{ textAlign: 'center', padding: '32px', color: 'var(--color-text-secondary)' }}>
                      No cases have been registered yet. This view will populate as FIRs are filed.
                    </td>
                  </tr>
                  ...
                  filteredRecords.map((r) => (
                    <tr key={r.case_id} style={{ cursor: 'pointer' }} onClick={() => setSelectedRecord(r)}>
                      <td><HashCell hash={r.case_hash} /></td>
                      <td style={{ fontFamily: 'var(--font-mono)', fontSize: '13px' }}>{r.case_number}</td>
                      <td style={{ fontWeight: 500 }}>{r.crime_type}</td>
                      <td><StatusChip status={r.investigation_status} label={r.investigation_status.replace(/_/g, ' ')} /></td>
                      <td style={{ fontVariantNumeric: 'tabular-nums' }}><strong>{r.days_active}</strong> days</td>
                      <td style={{ fontSize: '13px' }}>{r.court_level || '—'}</td>
                      <td>{r.bail_status ? <StatusChip status={r.bail_status} label={r.bail_status.replace(/_/g, ' ')} /> : <span style={{ color: 'var(--color-text-tertiary)' }}>—</span>}</td>
                    </tr>
                  ))
                )}
              </tbody>
        ...
        {selectedRecord && (
          <div
            style={{
              ...
              backgroundColor: 'rgba(11, 37, 71, 0.45)', display: 'flex',
          ...
          >
            <div
              className="card"
              style={{ width: '100%', maxWidth: '640px', maxHeight: '90vh', overflowY: 'auto', padding: '24px' }}
            ...
            >
              ...
              <div style={{ backgroundColor: 'var(--color-surface-subtle)', padding: '12px 14px', borderRadius: 'var(--radius)', border: '1px solid var(--color-border)', marginBottom: '16px' }}>
                ...
                <code style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', wordBreak: 'break-all' }}>{selectedRecord.case_hash}</code>
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', fontSize: '13px' }}>
                <div>
                  <span style={{ color: 'var(--color-text-secondary)', display: 'block', fontSize: '11px', textTransform: 'uppercase' }}>Investigation Stage</span>
                  <strong>{selectedRecord.investigation_status.replace(/_/g, ' ')}</strong>
                </div>
                <div>
                  <span style={{ color: 'var(--color-text-secondary)', display: 'block', fontSize: '11px', textTransform: 'uppercase' }}>Days Active</span>
                  <strong>{selectedRecord.days_active} days</strong>
                </div>
                <div>
                  <span style={{ color: 'var(--color-text-secondary)', display: 'block', fontSize: '11px', textTransform: 'uppercase' }}>Court Level</span>
                  <span>{selectedRecord.court_level || 'Not yet assigned'}</span>
                </div>
                <div>
                  <span style={{ color: 'var(--color-text-secondary)', display: 'block', fontSize: '11px', textTransform: 'uppercase' }}>Bail Status</span>
                  <span>{selectedRecord.bail_status ? selectedRecord.bail_status.replace(/_/g, ' ') : 'Not applicable'}</span>
                </div>
              </div>
