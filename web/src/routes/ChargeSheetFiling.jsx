...
export default function ChargeSheetFiling() {
  ...
  const handleAttemptFiling = async (e) => {
    ...
    if (!caseId) {
      setFilingStatus({
        type: "error",
        msg: "No case selected — open this page from a specific case.",
      });
      return;
    }
    ...
    try {
      ...
      if (
        err.status === 409 &&
        err.detail &&
        Array.isArray(err.detail.missing_items)
      ) {
        ...
        setFilingStatus({
          ...
          msg:
            err.detail.message ||
            "Charge sheet cannot be filed: mandatory stage requirements remain incomplete.",
        });
      } else {
        setFilingStatus({
          type: "error",
          msg: err.message || "Could not file the charge sheet.",
        });
      }
    } finally {
  ...
  return (
    <div>
      ...
      <div className="page-container">
        ...
        {filingStatus && (
          <div
            className={`alert ${filingStatus.type === "success" ? "alert-success" : "alert-error"}`}
          >
            {filingStatus.msg}
          </div>
        )}
        ...
        <div className="card" style={{ maxWidth: "560px" }}>
          ...
          >
            If any mandatory requirement is missing, filing is rejected and the
            specific missing items are listed above — nothing is filed
            partially.
          </p>
          ...
          <form onSubmit={handleAttemptFiling}>
            <div className="form-group">
              <label className="form-label">
                Prosecution Submissions & Grounds
              </label>
              <textarea
