...
export default function DocumentViewer() {
  ...
  const handleApplyCorrection = async (e) => {
    ...
    try {
      ...
      setCorrectionAlert({ type: 'success', msg: `Correction recorded: [${entityType}] at [${spanStart}:${spanEnd}]. Audit trail updated.` });
    } catch (err) {
      setCorrectionAlert({ type: 'error', msg: `Correction failed: ${formatError(err)}` });
    } finally {
  ...
  if (loadError || !documentData) {
    return (
      <div className="page-container" style={{ padding: '40px 24px' }}>
        <div className="alert alert-error">{loadError || 'Document not found.'}</div>
      ...
      <div className="page-container">
        ...
        <div className="page-header" style={{ marginBottom: '14px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <StatusChip status={doc.chain_status} label={`Ledger: ${(doc.chain_status || 'unknown').toUpperCase()}`} />
          ...
          <div className="card" style={{ padding: '20px' }}>
            ...
            {doc.download_url && (
              <div style={{ marginTop: '12px' }}>
                <a href={doc.download_url} target="_blank" rel="noreferrer" className="btn btn-secondary btn-sm">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div className="card">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                <div>
                  ...
                  <div style={{ marginTop: '4px' }}>
                    <StatusChip status={doc.retention_legal_hold ? 'critical' : 'neutral'} label={doc.retention_legal_hold ? 'Active' : 'None'} />
                  </div>
                </div>
            ...
            <div className="card">
              ...
              <p className="text-caption" style={{ marginBottom: '12px' }}>
                ...
                Restricted to the Investigating Officer assigned to this case.
              </p>

              {correctionAlert && (
                <div className={`alert ${correctionAlert.type === 'success' ? 'alert-success' : 'alert-error'}`} style={{ padding: '8px 10px', fontSize: '11px' }}>
                  {correctionAlert.msg}
                </div>
              )}
              ...
              <form onSubmit={handleApplyCorrection}>
                ...
                <button type="submit" className="btn btn-secondary btn-sm" style={{ width: '100%' }} disabled={submittingCorrection}>
                  {submittingCorrection ? 'Submitting...' : 'Submit Officer Correction'}
                </button>
              </form>
            </div>
