...
export default function PoliceInvestigation() {
  ...
  const handleRegisterFIR = async (e) => {
    ...
    try {
      ...
      setFirStatus({
        type: "error",
        msg: `FIR registration failed: ${formatError(err)}`,
      });
    }
  };
  ...
  const handleFileUpload = async (e) => {
    ...
    if (!uploadFile) {
      setUploadStatus({
        type: "error",
        msg: "Select a file before ingesting.",
      });
      return;
    }
    if (!selectedCaseId) {
      setUploadStatus({
        type: "error",
        msg: "No case selected — register or select a case first.",
      });
      return;
    }
    ...
    try {
      ...
      setUploadStatus({
        type: "error",
        msg: `Upload failed: ${formatError(err)}`,
      });
    }
  };
      ...
      <div className="page-container">
        <div className="page-header">
          <div>
            ...
            <p className="page-desc">
              First Information Report registration, evidentiary record
              ingestion, and case diary maintenance.
            </p>
          </div>
          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
            <StatusChip
              status="neutral"
              label={`Role: ${user?.role ? user.role.replace(/_/g, " ").toUpperCase() : "UNKNOWN"}`}
            />
          </div>
        </div>
            ...
            <span className="stat-value" style={{ color: "var(--ink-900)" }}>
              {
                cases.filter(
                  (c) =>
                    ...
                    c.investigation_status !== "FIR_Registered",
                ).length
              }
            </span>
        ...
        <div className="grid-2">
          ...
          <div className="card">
            ...
            >
              Creates the case record and stores this narrative as its first
              document in the same step — hashed and queued for redaction
              automatically. Restricted to the Duty Officer role.
            </p>

            {firStatus && (
              <div
                className={`alert ${firStatus.type === "success" ? "alert-success" : firStatus.type === "error" ? "alert-error" : "alert-warning"}`}
              >
                {firStatus.msg}
              </div>
            )}
              ...
              <div className="form-group">
                ...
                >
                  <option value="Domestic Violence">
                    Domestic Violence (Protection of Women / Sec 498A IPC)
                  </option>
                  <option value="Cybercrime">
                    Cybercrime (IT Act / Financial Cyberfraud)
                  </option>
                  <option value="NDPS">
                    NDPS (Narcotics & Psychotropic Substances)
                  </option>
                  <option value="Homicide">Homicide (BNS / Sec 302 IPC)</option>
                  <option value="Financial Fraud">
                    Financial Fraud & Money Laundering (PMLA)
                  </option>
                  <option value="Theft">
                    Theft & Burglary (Sec 379/380 IPC)
                  </option>
                  <option value="Robbery">
                    Armed Robbery & Dacoity (Sec 392 IPC)
                  </option>
                  <option value="Sexual Assault">
                    Sexual Assault & Rape (Sec 376 IPC / POCSO)
                  </option>
                  <option value="Acid Attack">
                    Acid Attack (Sec 326A IPC)
                  </option>
                  <option value="Road Accident">
                    Road Accident & Rash Driving (Sec 279/304A IPC)
                  </option>
                  <option value="Public Corruption">
                    Public Corruption & Bribery (PC Act)
                  </option>
                  <option value="Cyber Identity Theft">
                    Cyber Identity Theft (Sec 66C IT Act)
                  </option>
                  <option value="Organized Crime">
                    Organized Crime & Extortion (MCOCA / IPC 384)
                  </option>
                  <option value="Kidnapping">
                    Kidnapping & Abduction (Sec 363/364A IPC)
                  </option>
                  <option value="General Cognizable Offense">
          ...
          <div className="card">
            ...
            >
              For anything beyond the FIR narrative itself — Panchnama, forensic
              reports, CCTV/media, witness statements. Hashing and OCR/redaction
              run asynchronously, same as the FIR document.
            </p>

            {uploadStatus && (
              <div
                className={`alert ${uploadStatus.type === "success" ? "alert-success" : uploadStatus.type === "error" ? "alert-error" : "alert-warning"}`}
              >
                {uploadStatus.msg}
              </div>
            )}
            ...
            <form onSubmit={handleFileUpload}>
              ...
              <div className="form-group">
                ...
                >
                  ...
                  <option value="Panchnama">
                    Panchnama (Seizure / Scene of Crime)
                  </option>
                  <option value="Forensic_Report">
                    Forensic Analysis Report
                  </option>
                  <option value="CCTV_Footage">
                    Binary Media (CCTV / Phone Dump)
                  </option>
                  <option value="Witness_Statement">
                    Witness Statement (Section 161)
                  </option>
                </select>
              </div>
            ...
            {uploadedDoc && (
              ...
              >
                ...
                >
                  ...
                  <StatusChip
                    status={uploadedDoc.chain_status}
                    label={`Ledger: ${uploadedDoc.chain_status}`}
                  />
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  <div>
                    SHA-256:{" "}
                    {uploadedDoc.doc_hash ||
                      "(pending — hashing runs asynchronously)"}
                  </div>
                  <div>Status: {uploadedDoc.status}</div>
                </div>
                <Link
                  to={`/cases/${uploadedDoc.case_id}`}
                  style={{ fontSize: "12px" }}
                >
        ...
        <div className="card">
          <h2 className="card-title">
            Append Case Diary Entry (Section 172 CrPC / BNSS)
          </h2>
          <p
            style={{
              color: "var(--text-secondary)",
          ...
          >
            Day-to-day chronological record of investigation. Append-only.
            Restricted to the assigned IO/SHO; routes through the redaction
            pipeline before other roles can view it.
          </p>

          {diaryStatus && (
            <div
              className={`alert ${diaryStatus.type === "success" ? "alert-success" : diaryStatus.type === "error" ? "alert-error" : "alert-warning"}`}
            >
              {diaryStatus.msg}
            </div>
          )}
