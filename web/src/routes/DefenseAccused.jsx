...
export default function DefenseAccused() {
  ...
  useEffect(() => {
    ...
    apiClient(`/cases/${selectedCaseId}/bail`)
      .then((data) => {
        if (isMounted) setBailRecords(data || []);
      })
      .catch(() => {
        if (isMounted) setBailRecords([]);
      });
    return () => {
      isMounted = false;
    };
  ...
  const handleBailApplication = async (e) => {
    ...
    try {
      await apiClient(`/cases/${selectedCaseId}/bail/application`, {
        body: {},
      });
      setSubmissionAlert({
        type: "success",
      ...
      setSubmissionAlert({
        type: "error",
        msg: err.message || "Could not file the bail application.",
      });
    } finally {
  ...
  const handleSuretySubmission = async (e) => {
    ...
    try {
      ...
      setSubmissionAlert({
        type: "error",
        msg: err.message || "Could not register the surety undertaking.",
      });
    } finally {
  ...
  return (
    <div>
      ...
      <div className="page-container">
        <div className="page-header">
          <div>
            ...
            <p className="page-desc">
              Electronic submission gateway for bail petitions and surety
              undertakings, applied directly to the selected case's bail track.
            </p>
          </div>
        ...
        {submissionAlert && (
          <div
            className={`alert ${submissionAlert.type === "success" ? "alert-success" : "alert-error"}`}
          >
            {submissionAlert.msg}
          </div>
        )}
          ...
          <>
            <div className="grid-2">
              <div className="card">
                ...
                >
                  Moves this case's bail track from "Arrested" to "Application
                  Filed" — only valid once the accused has actually been
                  arrested on this case.
                </p>
                <button
              ...
              <div className="card">
                ...
                >
                  Only valid once the court has actually issued a bail order on
                  this case.
                </p>
                <form onSubmit={handleSuretySubmission}>
                  <div className="form-group">
                    <label className="form-label">
                      Surety Guarantor Full Name
                    </label>
                    <input
                      type="text"
            ...
            <div className="card" style={{ marginTop: "16px" }}>
              <span className="table-caption">
                Bail track history for this case.
              </span>
                  ...
                  <tbody>
                    {bailRecords.length === 0 ? (
                      <tr>
                        ...
                        >
                          No bail activity recorded for this case yet.
                        </td>
                      </tr>
                    ) : (
                      bailRecords.map((r) => (
