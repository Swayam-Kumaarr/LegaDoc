...
function useCaseResource(caseId, endpoint) {
  ...
  useEffect(() => {
    ...
    apiClient(endpoint)
      ...
      .catch((e) => {
        if (isMounted) setError(e.message || "Could not load this data.");
      })
      .finally(() => {
...
export default function CaseDetail() {
  ...
  const evidenceRes = useCaseResource(
    caseId,
    `/cases/${caseId}/evidence-requests`,
  );
  ...
  const auditEvents = auditLog?.entries || [];

  if (caseRes.loading) {
    return (
      <div className="page-container">
        <p className="text-body">Loading case…</p>
      </div>
    );
  }
    ...
    return (
      <div className="page-container">
        <div className="alert alert-error">
          {caseRes.error || "Case not found, or you do not have access to it."}
        </div>
      </div>
    );
  }
      ...
      <div className="page-container">
        <div className="page-header" style={{ marginBottom: "14px" }}>
          <div>
            ...
            >
              ...
              <StatusChip
                status={caseData.investigation_status}
                label={caseData.investigation_status.replace(/_/g, " ")}
              />
            </div>
          ...
          <Link
            to={`/cases/${caseData.id}/charge-sheet`}
          ...
          <div className="grid-2" style={{ margin: 0, gap: "12px" }}>
            ...
            <div>
              ...
              <div className="text-body">
                {caseData.bail_status
                  ? caseData.bail_status.replace(/_/g, " ")
                  : "Not applicable"}
              </div>
            </div>
          </div>
                  ...
                  {docsRes.error ? (
                    ...
                    documents.map((d) => (
                      <tr key={d.id}>
                        <td>
                          {d.doc_type}{" "}
                          <span className="text-caption">v{d.version}</span>
                        </td>
                        <td>v{d.version}</td>
                        <td>
                          <HashCell hash={d.doc_hash} />
                        </td>
                        <td>
                          <StatusChip
                            status={d.status}
                        ...
                        <td>
                          <StatusChip
                            ...
                            label={d.chain_status.replace(/_/g, " ")}
                          />
                        </td>
                        <td>
                          <Link
          ...
          <div className="card" style={{ padding: 0, overflow: "hidden" }}>
            ...
            >
              <span className="text-label">
                Section 91 CrPC Production Orders
              </span>
            </div>
            <div
              className="table-container"
                ...
                <tbody>
                  {evidenceRes.error ? (
                    ...
                    <tr>
                      ...
                      >
                        No evidence requests dispatched for this case yet.
                      </td>
                    </tr>
                  ) : (
        ...
        {activeTab === "bail" && (
          <div className="card" style={{ padding: 0, overflow: "hidden" }}>
            ...
            >
              <span className="text-label">
                Bail Track (Section 437/439 CrPC)
              </span>
            </div>
            <div
              className="table-container"
                ...
                <tbody>
                  {bailRes.error ? (
                    ...
                    <tr>
                      ...
                      >
                        No bail activity recorded for this case.
                      </td>
                    </tr>
                  ) : (
                    bailRecords.map((r) => (
        ...
        {activeTab === "diary" && (
          <div className="card">
            <h3 className="card-title">
              Daily Police Diary of Proceedings (Section 172 CrPC)
            </h3>
            {diaryRes.error ? (
              <p
                className="text-body"
                style={{ color: "var(--color-status-error, #b91c1c)" }}
              >
                {diaryRes.error}
              </p>
              ...
              >
                No diary entries recorded for this case yet.
              </p>
            ) : (
              <div
                style={{
                ...
                {diaryEntries.map((entry) => (
                  ...
                  >
                    ...
                    <div className="text-body" style={{ fontSize: "13px" }}>
                      {entry.status === "ready"
                        ? entry.text
                        : "[Pending AI Parser review — visible only to the assigned IO/SHO until cleared]"}
                    </div>
                  </div>
                ))}
              </div>
                ...
                {auditRes.error ? (
                  ...
                  <div style={{ padding: "16px" }}>
                    <p className="text-body" style={{ marginBottom: "10px" }}>
                      ...
                      {auditLog.first_entry_at
                        ? new Date(auditLog.first_entry_at).toLocaleDateString()
                        : "—"}{" "}
                      and{" "}
                      {auditLog.last_entry_at
                        ? new Date(auditLog.last_entry_at).toLocaleDateString()
                        : "—"}
                      .
                    </p>
                    <div
                    ...
                    >
                      {Object.entries(auditLog.action_counts || {}).map(
                        ([action, count]) => (
                          <span
                            key={action}
