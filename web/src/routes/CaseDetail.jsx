        ...
        {activeTab === 'audit' && (
          <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--color-border)' }}>
              ...
              {/* Three states, never collapsed into two. The previous
                  expression fell back to the literal 'VERIFIED' whenever
                  chain_intact was false, so a chain that failed verification
                  was reported to the user as having passed it — the exact
                  inversion this product exists to prevent. Unknown (no
                  response yet, or the field absent) is also not "verified". */}
              <span
                className="text-caption"
                style={auditData?.chain_intact === false ? { color: 'var(--color-status-danger, #b00020)', fontWeight: 600 } : undefined}
              >
                Ledger Chain Integrity: {
                  auditData?.chain_intact === true
                    ? 'VALID & UNBROKEN'
                    : auditData?.chain_intact === false
