...
const CREDENTIAL_DOC_TYPES = [
  ...
  {
    value: "bar_enrollment_certificate",
    label: "Bar Council Enrollment Certificate",
  },
  { value: "judicial_appointment_order", label: "Judicial Appointment Order" },
  {
    value: "institutional_authorization_letter",
    label: "Institutional Authorization Letter",
  },
  {
    value: "nabl_accreditation_certificate",
...
export default function OfficerOnboarding() {
  ...
  const fetchApplications = async () => {
    ...
    try {
      const data = await apiClient(
        `/admin/applications?status_filter=${statusFilter}`,
      );
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setCreateAlert({
        type: "error",
        msg: `Could not open application: ${formatError(err)}`,
      });
    }
  };
  ...
  const handleUploadDocument = async (e) => {
    ...
    try {
      await apiUpload(
        `/admin/applications/${selected.id}/credential-documents`,
        formData,
      );
      setUploadAlert({
        type: "success",
  ...
  const handleApprove = async () => {
    ...
    try {
      const res = await apiClient(
        `/admin/applications/${selected.id}/approve`,
        { method: "POST" },
      );
      ...
      setDecisionAlert({
        type: "error",
        msg: `Could not approve: ${formatError(err)}`,
      });
    }
  };
  ...
  const handleReject = async (e) => {
    ...
    setDecisionAlert(null);
    try {
      await apiClient(`/admin/applications/${selected.id}/reject`, {
      ...
      setDecisionAlert({
        type: "error",
        msg: `Could not reject: ${formatError(err)}`,
      });
    }
  };
      ...
      <div className="page-container">
        <div className="page-header">
          <div>
            ...
            <p className="page-desc">
              Verify credentials before provisioning an account. Every claimed
              identity is cross-checked against an uploaded proof document (OCR
              + extraction) — but the match result is advisory only; approval is
              always an explicit decision, never automatic.
            </p>
          </div>
          <button
        ...
        {showNewForm && (
          <div className="card">
            ...
            {createAlert && (
              <div
                className={`alert ${createAlert.type === "success" ? "alert-success" : "alert-error"}`}
              >
                {createAlert.msg}
              </div>
            )}
            ...
            >
              {applications.map((a) => (
                ...
                >
                  ...
                  >
                    {a.email} · claiming {a.claimed_role}
                  </div>
                  <div
                    style={{
                      marginTop: "4px",
                  ...
                  >
                    {a.documents.length === 0 && (
                      <StatusChip status="neutral" label="No documents yet" />
                    )}
                    {a.documents.map((d) => (
                      <StatusChip
                        key={d.id}
                        status={
                          (
                            MATCH_STATUS_LABEL[d.match_status] || {
                              status: "neutral",
                            }
                          ).status
                        }
                        label={
                          d.match_status
                            ? (MATCH_STATUS_LABEL[d.match_status] || {}).label
                            : d.status
                        }
                      />
                    ))}
          ...
          <div className="card">
            {!selected ? (
              ...
              <div>
                ...
                <h2 className="card-title" style={{ marginTop: "10px" }}>
                  Account Created
                </h2>
                <div className="alert alert-warning">
                  This temporary password is shown <strong>exactly once</strong>
                  . Communicate it to {approvedCredential.email} out-of-band now
                  — it cannot be retrieved again. They must change it on first
                  login.
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  <div>
                    Temporary Password:{" "}
                    <strong>{approvedCredential.temporary_password}</strong>
                  </div>
                </div>
              </div>
            ) : (
              <div>
                ...
                >
                  {selected.email} · claiming{" "}
                  <strong>{selected.claimed_role}</strong>
                  {selected.designation && <> · {selected.designation}</>}
                  <br />
                  Claimed credential ID:{" "}
                  <span className="mono-text">
                    {selected.claimed_credential_id || "—"}
                  </span>
                </div>
                  ...
                  <div style={{ marginBottom: "16px" }}>
                    ...
                    {selected.documents.map((d) => (
                      ...
                      >
                        ...
                        >
                          <strong>
                            {CREDENTIAL_DOC_TYPES.find(
                              (t) => t.value === d.doc_type,
                            )?.label || d.doc_type}
                          </strong>
                          <StatusChip
                            status={
                        ...
                        {d.extracted_fields && (
                          ...
                          >
                            <div>
                              Extracted name:{" "}
                              {d.extracted_fields.name || "(none found)"}
                            </div>
                            <div>
                              Extracted ID:{" "}
                              {d.extracted_fields.id_number || "(none found)"}
                            </div>
                            <div>
                              Name similarity to claim:{" "}
                              {Math.round(
                                (d.extracted_fields.name_similarity || 0) * 100,
                              )}
                              %
                            </div>
                          </div>
                ...
                {selected.status === "pending_review" && (
                  <>
                    ...
                    >
                      ...
                      {uploadAlert && (
                        <div
                          className={`alert ${uploadAlert.type === "success" ? "alert-success" : "alert-error"}`}
                          style={{ fontSize: "12px" }}
                        >
                          {uploadAlert.msg}
                        </div>
                    ...
                    {decisionAlert && (
                      <div
                        className={`alert ${decisionAlert.type === "success" ? "alert-success" : "alert-error"}`}
                      >
                        {decisionAlert.msg}
                      </div>
                    )}
